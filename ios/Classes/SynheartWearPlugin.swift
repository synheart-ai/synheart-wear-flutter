...
public class SynheartWearPlugin: NSObject, FlutterPlugin {
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    ...
    // Register HealthKit profile handler (DOB / sex / blood type / latest
    // height / weight / body-fat samples) for the host app's "Sync from
    // Apple Health" prefill in About You.
    if #available(iOS 13.0, *) {
      HealthKitProfileHandler.register(with: registrar)
    }
  }

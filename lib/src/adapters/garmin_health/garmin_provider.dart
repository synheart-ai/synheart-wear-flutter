...
class GarminProvider {
  ...
  // SSE subscription state removed — RAMEN is the sole real-time channel.
  ...
  Future<String?> handleDeepLinkCallback(Uri uri) async {
    ...
    if (isSuccess && userID != null && userID.isNotEmpty) {
      ...
      // by Ping webhooks, not OAuth tokens, and always reject with
      // 400 `Invalid pull token` here. The supported path is to ask
      // Garmin to push via /garmin/backfill/{summary_type}; data
  ...
  /// Fetch full workout / activity summaries (Garmin `activities` endpoint).
  /// Distinct from [fetchDailies] (rolled-up day) and [fetchEpochs] (15 min).
  /// Returns list of WearMetrics in unified format.
  ///
  /// Used by the workout-event adapter to build [WorkoutEvent]s for
  /// personalization-aware confidence modulation.
  ...
  Future<List<WearMetrics>> fetchSleeps({
    ...
    final response = await _fetchData('sleep', effectiveUserId, start, end);
    return _convertToWearMetricsList(response, 'garmin', effectiveUserId);
  }

  /// Fetch raw Garmin data in the canonical normalized format.
  /// Combines dailies and sleeps data into the engine's expected schema.
  /// userId is optional, uses stored userId if not provided.
  Future<Map<String, dynamic>> fetchRawData({
    ...
    final sleeps = await _fetchData('sleep', effectiveUserId, start, end);
    ...
    // Return in the canonical normalized format: { "dailies": [...], "sleep": [...] }

...
`flatDir { dirs 'libs' }` repositories don't propagate when this plugin is
consumed as a transitive Gradle library module (a common Flutter setup), so
the plugin resolves the licensed Garmin AAR via a real Maven coordinate
(`com.garmin.health:companion-sdk:4.7.0`) instead. That works in both
standalone apps and library consumers.

## Drop-in setup

1. Get the licensed AAR from your Garmin Health SDK distribution
   channel (Companion variant for direct-Bluetooth, Standard variant
   for use alongside Garmin Connect Mobile).
...
- [`../../doc/GARMIN_SETUP.md`](../../doc/GARMIN_SETUP.md) — full Garmin integration guide
